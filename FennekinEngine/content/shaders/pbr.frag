 ...
 */
float fnk_distributionGGX(float NdotH, float a) {
  float a2 = a * a;
 ...
 */
float fnk_visibilitySmithGGXCorrelated(float NdotV, float NdotL, float a) {
  float a2 = a * a;
 ...
 */
vec3 fnk_fresnelSchlick(float LdotH, vec3 F0) {
 ...
 */
vec3 fnk_fresnelSchlickRoughness(float LdotV, vec3 F0, float roughness) {
  return F0 + (max(vec3(1.0 - roughness), F0) - F0) * pow(1.0 - LdotV, 5.0);
}

// TODO: Move to an IBL shader file.

vec3 fnk_samplePrefilteredEnvMap(vec3 viewDir_worldSpace,
...
vec2 fnk_sampleBrdfLUT(vec3 viewDir_worldSpace, vec3 normal_worldSpace,
...
vec3 fnk_shadeAmbientIBLDeferred(vec3 albedo, vec3 irradiance,
  ...
  vec3 F = fnk_fresnelSchlickRoughness(clamp(dot(normal, viewDir), 0.0, 1.0),
                                       F0, roughness);
  vec3 kS = F;
  vec3 kD = 1.0 - kS;
  kD *= 1.0 - metallic;
...
vec3 fnk_shadeAmbientIrradiance(QrkMaterial material, vec2 texCoords,
  ...
  float metallic = fnk_extractMetallic(material, texCoords);
  float ao = fnk_extractAmbientOcclusion(material, texCoords);
  return fnk_shadeAmbientIBLDeferred(albedo, irradiance, prefilteredEnvColor,
 ...
 * attenuation.
 */
vec3 fnk_shadeCookTorranceGGXDeferred(vec3 albedo, float roughness,
  ...
  vec3 F = fnk_fresnelSchlick(LdotH, F0);
...
vec3 fnk_shadeCookTorranceGGX(QrkMaterial material, vec3 lightDiffuse,
  ...
  // :)
  return fnk_shadeCookTorranceGGXDeferred(albedo, roughness, metallic,
...
vec3 fnk_shadeDirectionalLightCookTorranceGGXDeferred(
  ...
  vec3 result = fnk_shadeCookTorranceGGXDeferred(albedo, roughness, metallic,
...
vec3 fnk_shadeDirectionalLightCookTorranceGGX(QrkMaterial material,
  ...
  float roughness = fnk_extractRoughness(material, texCoords);
  float metallic = fnk_extractMetallic(material, texCoords);
  return fnk_shadeDirectionalLightCookTorranceGGXDeferred(
...
vec3 fnk_shadePointLightCookTorranceGGX(QrkMaterial material,
  ...
  float roughness = fnk_extractRoughness(material, texCoords);
  float metallic = fnk_extractMetallic(material, texCoords);
  return fnk_shadePointLightCookTorranceGGXDeferred(albedo, roughness, metallic,
...
vec3 fnk_shadeSpotLightCookTorranceGGXDeferred(vec3 albedo, float roughness,
  ...
  float spotlightIntensity = fnk_calcSpotLightIntensity(light, lightDir);
...
vec3 fnk_shadeSpotLightCookTorranceGGX(QrkMaterial material, QrkSpotLight light,
  ...
  float roughness = fnk_extractRoughness(material, texCoords);
  float metallic = fnk_extractMetallic(material, texCoords);
  return fnk_shadeSpotLightCookTorranceGGXDeferred(albedo, roughness, metallic,
                                                   light, fragPos, normal);
}