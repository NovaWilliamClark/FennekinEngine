 ...
 * eye.
 */
float fnk_luminance(vec3 color) {
  return dot(color, vec3(0.2125, 0.7154, 0.0721));
}

/** Tone maps an HDR color using Reinhard tone mapping. */
vec3 fnk_toneMapReinhard(vec3 color) { return color / (color + 1.0f); }
 ...
 */
vec3 fnk_toneMapReinhardLuminance(vec3 color) {
  float luminance = fnk_luminance(color);
  vec3 mapped = fnk_toneMapReinhard(color);
 ...
 */
vec3 fnk_toneMapAcesApprox(vec3 color) {
  color *= 0.6;
  float a = 2.51;
  float b = 0.03;
  float c = 2.43;
...
float fnk_toneMapAMDTermB(float hdrMax, float contrast, float shoulder,
...
float fnk_toneMapAMDTermC(float hdrMax, float contrast, float shoulder,
                          float midIn, float midOut) {
...
float fnk_toneMapAMDFormula(float x, vec4 p) {
  float z = pow(x, p.r);
 ...
 */
vec3 fnk_toneMapAMD(vec3 color) {
  ...
  float b = fnk_toneMapAMDTermB(hdrMax, contrast, shoulder, midIn, midOut);
  float c = fnk_toneMapAMDTermC(hdrMax, contrast, shoulder, midIn, midOut);
  ...
  peak = fnk_toneMapAMDFormula(peak, vec4(contrast, shoulder, b, c));
