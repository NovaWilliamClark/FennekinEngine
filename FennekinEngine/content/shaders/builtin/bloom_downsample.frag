...
void main() {
  ...
  vec2 srcTexelSize = 1.0 / vec2(textureSize(fnk_bloomMipChain, 0));
  ...
  // clang-format off
  vec4 etl = texture(fnk_bloomMipChain, vec2(texCoords.x - 2*offsetX, texCoords.y + 2*offsetY));
  vec4 ett = texture(fnk_bloomMipChain, vec2(texCoords.x,             texCoords.y + 2*offsetY));
  vec4 etr = texture(fnk_bloomMipChain, vec2(texCoords.x + 2*offsetX, texCoords.y + 2*offsetY));

  vec4 ell = texture(fnk_bloomMipChain, vec2(texCoords.x - 2*offsetX, texCoords.y            ));
  vec4 ccc = texture(fnk_bloomMipChain, vec2(texCoords.x,             texCoords.y            ));
  vec4 err = texture(fnk_bloomMipChain, vec2(texCoords.x + 2*offsetX, texCoords.y            ));

  vec4 ebl = texture(fnk_bloomMipChain, vec2(texCoords.x - 2*offsetX, texCoords.y - 2*offsetY));
  vec4 ebb = texture(fnk_bloomMipChain, vec2(texCoords.x,             texCoords.y - 2*offsetY));
  vec4 ebr = texture(fnk_bloomMipChain, vec2(texCoords.x + 2*offsetX, texCoords.y - 2*offsetY));

  vec4 itl = texture(fnk_bloomMipChain, vec2(texCoords.x - offsetX, texCoords.y + offsetY));
  vec4 itr = texture(fnk_bloomMipChain, vec2(texCoords.x + offsetX, texCoords.y + offsetY));
  vec4 ibl = texture(fnk_bloomMipChain, vec2(texCoords.x - offsetX, texCoords.y - offsetY));
  vec4 ibr = texture(fnk_bloomMipChain, vec2(texCoords.x + offsetX, texCoords.y - offsetY));
  // clang-format on
