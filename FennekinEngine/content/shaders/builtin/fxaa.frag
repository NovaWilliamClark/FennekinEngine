...
void main() {
  vec2 texelSize = 1.0 / vec2(textureSize(fnk_screenTexture, 0));

  vec4 colorCenter = texture(fnk_screenTexture, texCoords);

  float lumaCenter = lumaFromGammaCompressed(colorCenter.rgb);

  // Luma at four direct cardinal neighbors.
  // clang-format off
  float lumaDown =  lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2( 0, -1)).rgb);
  float lumaUp =    lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2( 0,  1)).rgb);
  float lumaLeft =  lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2(-1,  0)).rgb);
  float lumaRight = lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2( 1,  0)).rgb);
  // clang-format on
  ...
  // clang-format off
  float lumaDownLeft =  lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2(-1, -1)).rgb);
  float lumaUpRight =   lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2( 1,  1)).rgb);
  float lumaUpLeft =    lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2(-1,  1)).rgb);
  float lumaDownRight = lumaFromGammaCompressed(textureOffset(fnk_screenTexture, texCoords, ivec2( 1, -1)).rgb);
  // clang-format on
  ...
  for (int i = 0; i < MAX_ITERATIONS; ++i) {
    if (!reached1) {
      lumaEnd1 =
          lumaFromGammaCompressed(texture(fnk_screenTexture, edgeCoords1).rgb);
      lumaEnd1 -= lumaLocalAverage;
    }
    if (!reached2) {
      lumaEnd2 =
          lumaFromGammaCompressed(texture(fnk_screenTexture, edgeCoords2).rgb);
  ...
  fragColor = texture(fnk_screenTexture, finalCoords);
}
