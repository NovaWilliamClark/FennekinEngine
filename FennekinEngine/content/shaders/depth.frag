 ...
 * and `far`.
 */
float fnk_linearizeDepth(float depth, float near, float far) {
 ...
 * `near` and `far` planes.
 */
vec4 fnk_depthColor(float near, float far) {
  float depth = fnk_linearizeDepth(gl_FragCoord.z, near, far);
