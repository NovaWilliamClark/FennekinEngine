...
struct ModelRenderOptions {
  ...
  // Rendering.
  ELightingModel lightingModel = ELightingModel::COOK_TORRANCE_GGX;
  ...
  ESkyboxImage skyboxImage = ESkyboxImage::KLOPPENHEIM;
  ...
  EToneMapping toneMapping = EToneMapping::ACES_APPROX;
  ...
  // Camera.
  ECameraControlType cameraControlType = ECameraControlType::ORBIT;
  float speed = 0;
  float sensitivity = 0;
  float fov = 0;
  float nearPlane = 0;
  ...
  // Debug.
  EGBufferVis gBufferVis = EGBufferVis::DISABLED;
...
inline void renderImGuiUI(ModelRenderOptions &opts, UIContext ctx) {
  ...
  if (ImGui::CollapsingHeader("Rendering")) {
    ...
    if (ImGui::TreeNode("Environment")) {
      ...
      ImGui::BeginDisabled(opts.lightingModel != ELightingModel::BLINN_PHONG &&
                           opts.useIBL);
    ...
    if (ImGui::TreeNode("Post-processing")) {
      ...
      imguiFloatSlider("Gamma", &opts.gamma, 0.01f, 8.0f, nullptr, EScale::LOG);
      ImGui::EndDisabled();
  ...
  if (ImGui::CollapsingHeader("Camera")) {
    ImGui::RadioButton("Fly controls",
                       ...
                       static_cast<int>(ECameraControlType::FLY));
    ImGui::SameLine();
    ImGui::RadioButton("Orbit controls",
                       reinterpret_cast<int *>(&opts.cameraControlType),
                       static_cast<int>(ECameraControlType::ORBIT));
    ...
    if (imguiFloatSlider("Near plane", &opts.nearPlane, 0.01, 1000.0, nullptr,
                         EScale::LOG)) {
      if (opts.nearPlane > opts.farPlane) {
        opts.farPlane = opts.nearPlane;
      }
    }
    if (imguiFloatSlider("Far plane", &opts.farPlane, 0.01, 1000.0, nullptr,
                         EScale::LOG)) {
...
inline void
loadSkyboxImage(ESkyboxImage skyboxImage, SkyboxMesh &skybox,
